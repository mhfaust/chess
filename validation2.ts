 ...
 * @param length The maximum length of a string the resulting validator will pass
 * @param errMessage The error message returned if a string passed to the resulting validator is too long
 * @returns 
 */
const maxLength = (length: number, errMessage: string) => makeValidationFn<string>(input => input.length <= length, errMessage);



/**
 * HOF for creating string validators asserting a minimum length.
 * @param length The minimum length of a string the resulting validator will pass
 * @param errMessage The error message returned if a string passed to the resulting validator is too short
 * @returns 
 */
const minLength = (length: number, errMessage: string) => makeValidationFn<string>(input => input.length >= length, errMessage);
 ...
 * @returns 
 */
const matches = (pattern: RegExp, errMessage: string) => makeValidationFn<string>(input => Boolean(input.match(pattern)), errMessage)


function combineValidators<T> (...simpleValidationFns: FormValidationFn<T>[]): FormValidationFn<T> {
    return (input:T) => {
        const [isValid, errMessage]  = simpleValidationFns.map(v => (v(input))).find(errStr => Boolean(errStr)) || [true, null];
